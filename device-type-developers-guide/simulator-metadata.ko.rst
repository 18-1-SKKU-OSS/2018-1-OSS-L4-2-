...
이러한 메소드는 응답 선언의 첫번째 인수에 있는 값을 반환합니다. 응답 선언의 두번째 인수는 상태 선언과 동일한 방식으로 작동하며, 이들 인수에서 구문 분석 방법으로 보낸 메시지를 정의합니다. 하지만 이 경우에는 물리적인 작동이 아니라 명령에 따라 움직입니다.
...
``status()`` 메소드를 사용하면 물리적 작동을 시뮬레이션할 수 있으며, ``reply()`` 메소드를 사용하면 허브의 명령에 대응하여 장치로 메시지를 보내는 것을 시뮬레이션할 수 있습니다.
