...
- 유용성에 부정적인 영향을 미치고 불필요한 복합문제를 발생시킬 수 있으므로 여러 단계의 부모-자식 관계를 만드는 것에 주의하시길 바랍니다.
- 부모와 자식 SmartApp 사이에서 ``state`` 또는 ``atomicState`` 을 공유하는 것은 현재 지원되지 않습니다.
- SmartApp이 가질 수 있는 자식 앱의 개수는 :참고:`parent_child_count_limit` 에 설명된대로 제한됩니다.
...
부모-자식 관계는 개별 디바이스에서 독립적으로 작동하는 여러 자동화를 제공하려 할 때 유용할 수 있습니다. 부모 SmartApp은 여러 자식 앱을 가질 수 있고, 한 자식 SmartApp은 오직 하나의 부모 앱만 가질 수 있습니다.

부모-자식 관계를 만들기 위해서, 부모가 될 SmartApp은 ``app`` 입력 값을 사용해 자식이 될 앱을 지정해야 합니다. 자식 SmartApp은 부모가 될 SmartApp을 지정하는 ``parent`` 옵션을 지정해야 합니다.

부모 SmartApp은 찾으려는 앱의 이름을 안다면 ``getChildApps()`` 또는 ``findChildAppByName()`` 을 사용해 자식 앱을 가져올 수 있습니다. 자식 앱은 ``parent`` 속성을 통해 부모를 참조할 수 있습니다.
